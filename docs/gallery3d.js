(() => {
  ...
  function thumbnails(){q('g3-thumbs').replaceChildren();const start=Math.max(0,Math.min(index-4,list.length-10));for(const item of list.slice(start,start+10)){const b=document.createElement('button');b.dataset.id=item.id;b.setAttribute('aria-label',title(item));const img=document.createElement('img');img.src=thumbSrc(item);img.alt='';img.loading='eager';img.decoding='async';img.fetchPriority='high';const caption=document.createElement('span');caption.textContent=number(list.indexOf(item));b.append(img,caption);b.onclick=()=>{choose(list.indexOf(item));thumbnails();};q('g3-thumbs').append(b);}window.SeerFallback.watchImages();}
